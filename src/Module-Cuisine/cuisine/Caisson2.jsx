 ...
 * - Integrated equipment logic (Oven/Microwave, Sink, Cooktop, Dishwasher)
...
import { Dishwasher } from "./Dishwasher"; // <-- NOUVEAU: Import du lave-vaisselle
...
export const Caisson2 = ({
	...
	const isDishwasher = equipement === "dishwasher_45" || equipement === "dishwasher_60"; // <-- NOUVEAU FLAG
	...
	if (isIsland) {
		...
		return (
			<group position={[w / 2, h / 2, d / 2]}>
				...
				<mesh position={[0, bottomY + plintheH / 2, bodyZ - 0.02]}>
					<boxGeometry args={[w - 0.04, plintheH, bodyD - 0.04]} />
					<primitive object={facadeMaterial} attach="material" />
				</mesh>
				<mesh position={[0, bottomY + plintheH + corpsH / 2, bodyZ]}>
					<boxGeometry args={[w - 0.042, corpsH - 0.002, bodyD - 0.022]} />
					<meshStandardMaterial color={couleurInt} roughness={0.9} />
				</mesh>
				<mesh position={[0, bottomY + plintheH + corpsH / 2, bodyZ - bodyD / 2 - 0.01]}>
					<boxGeometry args={[w, corpsH, 0.02]} />
					<meshStandardMaterial color={couleurInt} roughness={0.3} />
				</mesh>
				<mesh position={[-w / 2 + 0.01, bottomY + plintheH + corpsH / 2, bodyZ]}>
					<boxGeometry args={[0.02, corpsH, bodyD]} />
					<meshStandardMaterial color={couleurInt} roughness={0.3} />
				</mesh>
				<mesh position={[w / 2 - 0.01, bottomY + plintheH + corpsH / 2, bodyZ]}>
					<boxGeometry args={[0.02, corpsH, bodyD]} />
				...
				{Array.from({ length: nbCols }).map((_, i) => {
					...
					return (
						<group key={i}>
							<mesh position={[cx, bottomY + plintheH + corpsH * 0.85, bodyZ + bodyD / 2 + doorThickness / 2 + 0.002]}>
								<boxGeometry args={[colW, corpsH * 0.3 - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
							</mesh>
							<mesh position={[cx, bottomY + plintheH + corpsH * 0.35, bodyZ + bodyD / 2 + doorThickness / 2 + 0.002]}>
								<boxGeometry args={[colW, corpsH * 0.7 - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
							</mesh>
							{avecPoignees && (
								<>
									<mesh position={[cx, bottomY + plintheH + corpsH * 0.85 + 0.05, bodyZ + bodyD / 2 + doorThickness + 0.015]}>
										<boxGeometry args={[colW * 0.5, 0.01, 0.02]} />
										<primitive object={handleMaterial} attach="material" />
									</mesh>
									<mesh position={[cx, bottomY + plintheH + corpsH * 0.35 + 0.25, bodyZ + bodyD / 2 + doorThickness + 0.015]}>
										<boxGeometry args={[colW * 0.5, 0.01, 0.02]} />
										<primitive object={handleMaterial} attach="material" />
									</mesh>
								</>
	...
	return (
		<group position={[w / 2, h / 2, d / 2]}>
			{/* Si c'est un Lave-Vaisselle, on affiche le composant dédié, 
                sinon on affiche le caisson normal avec ses portes/tiroirs */}
			{isDishwasher ? (
				...
				<>
					...
					{isTall && equipement === "oven_microwave" ? (
						<group>
							<mesh position={[0, bottomY + plintheH + bottomDoorH / 2, doorZ]}>
								<boxGeometry args={[w - gap, bottomDoorH - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
							</mesh>
							{avecPoignees && (
								<mesh position={[0, bottomY + plintheH + bottomDoorH - 0.06, doorZ + 0.015]}>
							...
							<group position={[0, bottomY + plintheH + bottomDoorH + ovenH / 2, doorZ]}>
								<mesh>
									<boxGeometry args={[w - gap, ovenH - gap, doorThickness]} />
									<meshStandardMaterial color="#111827" />
								</mesh>
								<mesh position={[0, -0.05, 0.004]}>
									<boxGeometry args={[w * 0.8, ovenH * 0.6, doorThickness]} />
							...
							<group position={[0, bottomY + plintheH + bottomDoorH + ovenH + mwH / 2, doorZ]}>
								<mesh>
									<boxGeometry args={[w - gap, mwH - gap, doorThickness]} />
									<meshStandardMaterial color="#111827" />
								</mesh>
								<mesh position={[-w * 0.1, 0, 0.004]}>
									<boxGeometry args={[w * 0.6, mwH * 0.7, doorThickness]} />
									<meshStandardMaterial color="#000000" metalness={0.8} roughness={0.1} />
								</mesh>
								<mesh position={[w * 0.35, 0, 0.004]}>
									<boxGeometry args={[w * 0.15, mwH * 0.7, doorThickness]} />
							...
							<mesh position={[0, bottomY + plintheH + bottomDoorH + ovenH + mwH + topDoorH / 2, doorZ]}>
								<boxGeometry args={[w - gap, topDoorH - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
							</mesh>
							{avecPoignees && (
								<mesh position={[0, bottomY + plintheH + bottomDoorH + ovenH + mwH + 0.06, doorZ + 0.015]}>
									<boxGeometry args={[w * 0.4, 0.01, 0.02]} />
									<primitive object={handleMaterial} attach="material" />
								</mesh>
							)}
						...
						// ======================== //
						<group>
							<mesh position={[0, bottomY + plintheH + corpsH / 2, doorZ]}>
								<boxGeometry args={[w - gap, corpsH - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
								<Edges color="#000000" opacity={0.1} transparent />
							</mesh>
							{avecPoignees && (
								<mesh position={[0, bottomY + plintheH + corpsH / 2 + (isBase ? corpsH / 2 - 0.06 : -corpsH / 2 + 0.06), doorZ + 0.015]}>
									<boxGeometry args={[w * 0.4, 0.01, 0.02]} />
									<primitive object={handleMaterial} attach="material" />
								</mesh>
							)}
						...
						// ======================== //
						<group>
							<mesh position={[0, bottomY + plintheH + t3H + t2H + t1H / 2, doorZ]}>
								<boxGeometry args={[w - gap, t1H - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
								<Edges color="#000000" opacity={0.1} transparent />
							</mesh>
							{avecPoignees && (
								<mesh position={[0, bottomY + plintheH + t3H + t2H + t1H / 2, doorZ + 0.015]}>
									<boxGeometry args={[w * 0.4, 0.01, 0.02]} />
									<primitive object={handleMaterial} attach="material" />
								</mesh>
							)}

							<mesh position={[0, bottomY + plintheH + t3H + t2H / 2, doorZ]}>
								<boxGeometry args={[w - gap, t2H - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
								<Edges color="#000000" opacity={0.1} transparent />
							</mesh>
							{avecPoignees && (
								<mesh position={[0, bottomY + plintheH + t3H + t2H / 2 + 0.1, doorZ + 0.015]}>
									<boxGeometry args={[w * 0.4, 0.01, 0.02]} />
									<primitive object={handleMaterial} attach="material" />
								</mesh>
							)}

							<mesh position={[0, bottomY + plintheH + t3H / 2, doorZ]}>
								<boxGeometry args={[w - gap, t3H - gap, doorThickness]} />
								<primitive object={facadeMaterial} attach="material" />
								<Edges color="#000000" opacity={0.1} transparent />
							</mesh>
							{avecPoignees && (
								<mesh position={[0, bottomY + plintheH + t3H / 2 + 0.1, doorZ + 0.015]}>
									<boxGeometry args={[w * 0.4, 0.01, 0.02]} />
									<primitive object={handleMaterial} attach="material" />
								</mesh>
							)}
			...
			{/* ======================== */}
			{/* On rend toujours le plan de travail, même pour le lave-vaisselle ! */}
			{isBase && (
