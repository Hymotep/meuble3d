...
export const renderEtagere = (
    ...
    const innerWidth = largeur - EPAISSEUR * 2;
...
export const getActiveConfigElements = (activeConfig, renderEtagereFn, renderPenderieFn, renderTiroirsFn, hauteur) => {
    const elements = [];
    ...
    if (activeConfig === 2) {
        elements.push(<group key="group-biblio">{[1, 2, 3, 4].map((i) => renderEtagereFn((hauteur / 5) * i, `etagere-biblio-${i}`))}</group>);
    }
