...
const SelectionBottomBar = () => {
    ...
    const getEquipmentOptions = () => {
        if (selectedItem.type === "tall_cabinet") {
            return <><option value="none">Aucun</option><option value="oven_microwave">Fours</option></>;
        }
        return <><option value="none">Libre</option><option value="sink">Évier</option><option value="cooktop">Plaque</option></>;
    };
                ...
                <>
                    ...
                    <div style={theme.bottomBarSection}>
                        ...
                        <div style={{ display: "flex", gap: "12px", alignItems: "center", height: "31px" }}>
                            ...
                            <label style={{ ...theme.checkboxWrapper, border: "none", padding: 0 }}>
                                <input type="checkbox" checked={selectedItem.config.useTextures || false} onChange={(e) => updateItemConfig(selectedId, { useTextures: e.target.checked })} style={theme.checkbox} />
                                <span style={{ ...theme.label, margin: 0, fontSize: "12px" }}>Textures PBR</span>
                            </label>

                            {(selectedItem.type === "base_cabinet" || selectedItem.type === "island" || selectedItem.type === "tall_cabinet") && (
                                <select value={selectedItem.config.equipement || "none"} onChange={(e) => updateItemConfig(selectedId, { equipement: e.target.value })} style={{ ...theme.select, width: "110px", height: "31px", padding: "0 10px" }}>
                                    {getEquipmentOptions()}
                                </select>
                            )}
                        </div>
