        ...
        if (active && meshRef.current) {
            ...
            otherItems.forEach((other) => {
                ...
                if (Math.abs(newZ - oZ) < 0.1 && Math.abs(baseY - oY) < 0.1) {
                    ...
                    if (Math.abs(newX - targetRight) < SNAP_DISTANCE) { newX = targetRight; newZ = oZ; }
                    const targetLeft = oX - oW / 2 - worldW / 2;
                    if (Math.abs(newX - targetLeft) < SNAP_DISTANCE) { newX = targetLeft; newZ = oZ; }
                }

                const isPerp = Math.abs((rotationDeg % 180) - (otherRot % 180)) === 90 &&
                    (type === "base_cabinet" || type === "tall_cabinet") &&
                    (other.type === "base_cabinet" || other.type === "tall_cabinet");
                const gap = isPerp ? 0.05 : -0.005;

                const myMinX = newX - worldW / 2; const myMaxX = newX + worldW / 2;
                const myMinY = baseY; const myMaxY = baseY + h;
                const myMinZ = newZ - worldD / 2; const myMaxZ = newZ + worldD / 2;
                const oMinX = oX - oW / 2; const oMaxX = oX + oW / 2;
                const oMinY = oY; const oMaxY = oY + oH;
                const oMinZ = oZ - oD / 2; const oMaxZ = oZ + oD / 2;
