...
import { PERP_GAP, MIN_CABINET_GAP } from "../config/constants.js";
...
export const checkCollision = (movingItem, newX, newZ, otherItems, config) => {
    ...
    const iNeedClearance =
        config?.equipement === "oven_microwave" ||
        config?.isTiroirsInterieurs;

    const myClearance = iNeedClearance
        ? getClearanceBBox(movingItem, newX, newZ)
        : null;
    ...
    for (const other of otherItems) {
        ...
        const heightOverlap = Math.abs(newX / 1000 - oX) < 0.1 &&
            Math.abs(newZ - oZ) < SNAP_THRESHOLD &&
            Math.abs(movingItem.position[1] / 1000 - oY) < SNAP_THRESHOLD;
        ...
        if (other.config?.equipement === "oven_microwave" ||
            other.config?.isTiroirsInterieurs) {
